...
| `%USERPROFILE%\.cursor\rules\answer-format.mdc` | 回答格式：先简述再详细；表格/Mermaid 优先 |
...
└── rules/
    ...
    ├── answer-format.mdc             # alwaysApply: true
    └── commit-history.mdc            # alwaysApply: true（新增，硬性规范）
```
...
3. 从用户级目录**复制**上述四个 `.mdc` 到项目 `.cursor/rules/`（逐份按内容比对：与用户级源文件完全一致则跳过；缺失或内容不一致则覆盖同步——只判断一致与否，不判断新旧）。
4. 保持 frontmatter：`alwaysApply: true`；不要删改纪律正文。
5. 在 `AGENTS.md` / `CLAUDE.md`（若本 Phase 会写）中声明项目规则资产。
6. **存量业务项目**走 Lite 模式时：本 Step 仍**不可跳过**；其余步骤按 Lite 范围执行。
...
| 顺序 | Skill / 动作 | 作用 |
|---:|---|---|
| 0 | **Cursor MDC Rules（§2.1）** | 从用户级同步 `windows-path-discipline.mdc` + `windows-shell-discipline.mdc` + `answer-format.mdc` + `commit-history.mdc` 到项目 `.cursor/rules/` |
...
- [ ] `.cursor/rules/answer-format.mdc` 已落盘且 `alwaysApply: true`
...
- **端口**：取 `port-registry` 的「README preview shell」段（如 `8090–8099`），每主题唯一，写进主题 `CLAUDE.md` 与 `port-registry`。
...
- **配置本地化**：`window.__PREVIEW_README__` 的标题 / 提示 / 页脚随主题改；页脚指向 `src/website-preview/` 与 `assets/images/readme/`。
...
## Phase A — 资产 · 融合 · 对齐
0. 同步 Cursor MDC Rules（§2.1）：从用户级复制 `windows-path-discipline.mdc` + `windows-shell-discipline.mdc` + `answer-format.mdc` + `commit-history.mdc` 到项目 `.cursor/rules/`。
...
### 本仓 workbench 新建主题（agentic-workbench）

| 步骤 | 做法 |
|---|---|
| 1 | 在 `workbench/README.md` 看清 Kind（**资产** vs **模板**），复制**同类 peer** 骨架 |
| 2 | 对齐本文件的五件套与 `docs/` · `assets/` · 产品层；排除 `node_modules` 等 |
| 3 | 在 `docs/output/prd/port-registry.md` 登记端口（文首计数与主题清单一并改） |
| 4 | 更新 `workbench/README.md` 主题表 |

**不做**：恢复根级 `workbench/_template/` 生成器（ADR 0016 明确延期；复制 peer 足够前不引入共享脚手架）。模板主题勿把上游内部 AGENTS 提升进根治理。
...
- [ ] `.cursor/rules/` 四份 MDC 已落盘且 `alwaysApply: true`（含 `commit-history.mdc`）
...
| 三份 MDC（Windows + answer-format） | **四份 MDC**（+ `commit-history.mdc` 硬性规范） |
